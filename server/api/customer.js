...
// new products (latest by creation date)
router.get('/products/new', async function (req, res) {
  try {
    const products = await ProductDAO.selectNew(8);
    res.json(products);
  } catch (err) {
...
// hot products (random sample)
router.get('/products/hot', async function (req, res) {
  try {
    const products = await ProductDAO.selectHot(8);
    res.json(products);
  } catch (err) {
...
router.get('/products/category/:id', async function (req, res) {
  try {
    const products = await ProductDAO.selectByCategory(req.params.id);
    res.json(products);
  } catch (err) {
...
router.put('/update', async function (req, res) {
  ...
  const result = await CustomerDAO.update(id, updateData);

  if (result) {
    res.json({ success: true, message: 'Update success' });
  } else {
    res.json({ success: false, message: 'Update failure' });
  }
});
